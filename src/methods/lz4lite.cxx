...
std::string_view Lz4Lite::name() const noexcept {
  return "Lz4Lite";
}

CompressionMethod Lz4Lite::method() const noexcept {
  return CompressionMethod::Lz4Lite;
}
...
bool Lz4Lite::decompress(std::span<const uint8_t> input, std::vector<uint8_t>& output) const {
  ...
  while (src < src_end) {
    ...
    for (size_t i = 0; i < match_length; i++) { output.push_back(output[start_idx + i]); }
  }
